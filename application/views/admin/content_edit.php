<?php defined('BASEPATH') OR exit('No direct script access allowed');
    ...
    if($this->input->post()){
        ...
        $content_image      = set_value('userfile');
        $content_text       = set_value('content_text');
        $animate            = set_value('content_animate');
    } else {
        $section_id         = $content_list->section_id;
        $section_name       = $content_list->section_name; 
        $content_title      = $content_list->content_title;
        $content_image      = $content_list->content_image;
        $content_text       = $content_list->content_text;
        $animate            = $content_list->animate;
    }
...
<div class="box box-info">
	...
	<form class="form-horizontal" id="validation-form" role="form" method="post" enctype="multipart/form-data" action="<?php echo base_url(); ?>admin/content/edit/<?php echo $content_list->content_id; ?>">
		<div class="box-body">
		            ...
		            <div class="clearfix">
		                <select name="section_id" class="chosen-select" id="form-field-select-3" data-placeholder="Choose a section...">
		                    <?php foreach ($section->result() as $row): ?>
                                <option value="<?php echo $row->section_id; ?>" <?php if($section_id == $row->section_id) {echo"selected='selected'";} ?> ><?php echo $row->section_name; ?></option>
                            <?php endforeach;?>
		                </select>
		            </div>
		        </div>
				...
				<div class="col-xs-12 col-sm-8">
                    <div class="clearfix">
                        <div class="col-xs-12 col-sm-5">  
                            <?php if(empty($content_list->content_image)): ?>
                                <h5><span class="text-red">Empty default image</span></h5>
                            <?php else: ?>
                                <img src="<?php echo base_url(); ?><?php echo $content_image; ?>" width="100%">
                                <label>
                        ...
                        <div class="col-xs-12 col-sm-7">
                            <input type="file" name="userfile" id="id-input-file-3" class="col-xs-12 col-sm-6" />
			...
			<div class="form-group">
		        ...
		        <div class="col-xs-12 col-sm-9">
		            <div class="clearfix">
		                <textarea name="content_text" id="editor1" class="form-required"><?php echo $content_text; ?></textarea>
		            </div>
		        </div>
		    </div>
		        ...
		        <div class="col-xs-12 col-sm-4">
		            <div class="clearfix">
		                <select name="content_animate" class="input input--dropdown js--animations chosen-select2" id="animate" data-current-selected-option="<?php echo $this->input->post("content_animate"); ?>">
                            ...
                            <?php else: ?>
                                <option value="<?php echo $animate; ?>"><?php echo $animate;?></option>
                                <option value="">-- choose your animation --</option>
                            <?php endif; ?>
        					...
        					<table class="table table-striped">
        						<tbody class="thin-border-bottom">
	                                <tr>
	                                    ...
	                                    <td>
                                            <?php if($content_list->display_title_content == '1') { $dtc_checked = 'checked="checked"'; } else { $dtc_checked = ''; } ?>
	                                    	<input type="checkbox" name="display_title_content" data-toggle="toggle" data-on="Yes" data-off="No" data-onstyle="primary" data-offstyle="default" data-size="mini" <?php echo $dtc_checked; ?> />
	                                    </td>
	                                </tr>
	                                <tr>
	                                    <td>Animation repeat</td>
	                                    <td>
                                            <?php if($content_list->animation_repeat == '1') { $ar_checked = 'checked="checked"'; } else { $ar_checked = ''; } ?>
	                                        <input type="checkbox" name="animation_repeat" data-toggle="toggle" data-on="Yes" data-off="No" data-onstyle="primary" data-offstyle="default" data-size="mini" <?php echo $ar_checked; ?> />
	                                    </td>
	                                </tr>
	                            </tbody>
        					</table>
...
<script type="text/javascript">
    jQuery(function($) {
    	...
    	{
    		$('iframe#editor1_ifr').removeAttr('title');
    	}
